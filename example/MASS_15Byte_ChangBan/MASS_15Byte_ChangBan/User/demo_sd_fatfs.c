...
static void WriteFileTest(void)
{
	...
	if (err == 0)
	{
		...
		printf("\r\n  写耗时 : %dms   平均写速度 : %dB/S (%dKB/S)\r\n", timelen, (TEST_FILE_LEN * 1000) / timelen, ((TEST_FILE_LEN / 1024) * 1000) / timelen);
	}
...
//  char Tmp1[8];
  ...
  /* 打开文件 */
	sprintf(path, "%sSig.txt", DiskPath);
	result = f_open(&file, path, FA_READ);
  
	if (result == FR_OK)
	{
		printf("Sig.txt 文件打开成功\r\n");
	}
	else
	{
		printf("Sig.txt 文件打开失败  (%s)\r\n", FR_Table[result]);
	}
...
//  printf("Start\r\n");
//  for(uint32_t i=0;i<6144*2;i++)
//  {
//    f_lseek(&file,7*i);
//    f_read(&file,Tmp,7,&bw);
//    strncpy(&Tmp1[0],&Tmp[0],5);
//    g_MCU_ADC_Buf1[i] = atoi(Tmp1);
//  }
//  printf("End\r\n");
  
  f_read(&file,g_MCU_ADC_Buf1,DataSize,&bw);

  f_close(&file);
  printf("Start\r\n");
  for(uint32_t i=0;i<2*6144;i++)
  {
    strncpy(&Tmp[0],&g_MCU_ADC_Buf1[8*i],6);
    g_MCU_ADC_Buf2[i] = atof(Tmp);
  }
  printf("End\r\n");
}
