...
//	RTC_Handler.Init.AsynchPrediv=0X3;           //RTC异步分频系数(1~0X7F)      4
//  RTC_Handler.Init.SynchPrediv=0X1FFF;            //RTC同步分频系数(0~7FFF)   8192
//	RTC_Handler.Init.AsynchPrediv=7;           //RTC异步分频系数(1~0X7F)      8
//  RTC_Handler.Init.SynchPrediv=4095;            //RTC同步分频系数(0~7FFF)   4096
  ...
  { 
      RTC_Set_Time(12,0,0,RTC_HOURFORMAT12_AM);	        	//设置时间 ,根据实际时间修改
...
{ 
    ...
    HAL_NVIC_SetPriority(RTC_WKUP_IRQn,0x03,0x02); //抢占优先级1,子优先级2
...
void TimeUpDate(uint8_t Month,uint8_t Day,uint8_t Hour,uint8_t Minute,uint8_t Second)
{
  RTC_Set_Time(Hour,Minute,Second,RTC_HOURFORMAT12_AM);	        	//设置时间 ,根据实际时间修改
  RTC_Set_Date(23,Month,Day,1);		                    			//设置日期
}
