...
__align(32) __attribute__((at(0x38000000))) uint8_t g_spiTxBuf[SPI_BUFFER_SIZE];
__align(32) __attribute__((at(0x38006000))) uint8_t g_spiRxBuf[SPI_BUFFER_SIZE];//SRAM4已经被SPI数据缓冲区占用了48Kbyte
...
void bsp_InitSPIParam(uint32_t _BaudRatePrescaler, uint32_t _CLKPhase, uint32_t _CLKPolarity)
{
	...
	hspi.Init.Direction         = SPI_DIRECTION_2LINES_RXONLY;         /* 全双工 */
	hspi.Init.CLKPhase          = SPI_PHASE_2EDGE;             		/* 配置时钟相位 */
	hspi.Init.CLKPolarity       = SPI_POLARITY_LOW;           		/* 配置时钟极性 */
	hspi.Init.DataSize          = SPI_DATASIZE_8BIT;      	 	/* 设置数据宽度 */
	hspi.Init.FirstBit          = SPI_FIRSTBIT_LSB;         	/* 数据传输先传高位 */
	...
	hspi.Init.MasterInterDataIdleness = SPI_MASTER_INTERDATA_IDLENESS_00CYCLE; /* MIDI, 两个连续数据帧之间插入的最小时间延迟，单位SPI时钟周期个数 */
...
void HAL_SPI_MspInit(SPI_HandleTypeDef *_hspi)
{
	...
	hdma_rx.Init.Priority            = DMA_PRIORITY_VERY_HIGH;      /* 优先级 */
	...
	/* 配置DMA接收中断 */
	HAL_NVIC_SetPriority(SPIx_DMA_RX_IRQn, 5, 0);//注释：DMA传输错误或者传输完成都会触发中断
	HAL_NVIC_EnableIRQ(SPIx_DMA_RX_IRQn);
	
	/* 配置SPI中断 */
	HAL_NVIC_SetPriority(SPIx_IRQn, 5, 0);
//	HAL_NVIC_SetPriority(SPIx_IRQn, 4, 0);
...
void bsp_spiTransfer(void)
{
	...
	{
		if(HAL_SPI_Receive_DMA(&hspi, (uint8_t *)g_spiRxBuf, g_spiLen) != HAL_OK)	
		{
				Error_Handler(__FILE__, __LINE__);
			
		}   
		memcpy(g_CacheB,g_spiRxBuf,g_ReceiveBlock*4);
		memset(g_spiRxBuf,0,SPI_BUFFER_SIZE);

		if(g_IntMode==Receive)
		{
			int31_to_float(g_CacheB,g_ReceiveBlock,g_Cachef32);
			SynPickUp();
			IWDG_Feed();//喂狗
		}
		else if(g_IntMode==Storage)
		{
			if(g_SaveCnt<g_StoreBlockNum-1)
			{
				memmove(&g_StoreDataBuf[g_SaveCnt*g_ReceiveBlock],&g_CacheB[0],g_ReceiveBlock*4);
				g_SaveCnt++;
				
			}
			else if(g_SaveCnt==g_StoreBlockNum-1)
			{
				memmove(&g_StoreDataBuf[g_SaveCnt*g_ReceiveBlock],&g_CacheB[0],g_ReceiveBlock*4);
				g_SaveCnt=0;
				g_StorageDataOK=0;	
		...
		else if(g_IntMode==SaveTest)
		{
//			SDWriteData6144(&g_FrameSig[g_SaveCntTest*6144]);
//			g_SaveCntTest++;
			g_StorageDataOK=0;
		}
		else
		{
			IdleCounter++;
			if(IdleCounter>10000000)IdleCounter=0;
		}	
		memset(&g_CacheB[0],0,g_ReceiveBlock*g_ADBit);		
//	printf("%d\r\n",4);
	}
//	ItCounter++;
//	if(ItCounter>20)
...
//		g_recount = 0;
//	}
//	g_recount = g_recount+1;
//	HAL_GPIO_TogglePin(GPIOJ, GPIO_PIN_4);
	wTransferState = TRANSFER_COMPLETE;
	bsp_spiTransfer();
//	printf("\n进入HAL_SPI_RxCpltCallback");
}
