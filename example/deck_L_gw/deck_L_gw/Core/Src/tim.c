...
void MX_TIM4_Init(void)
{
  ...
  /* USER CODE BEGIN TIM4_Init 0 */
	// OC模式下   频率需再除2    实际频率 = 240 000 000 / 10 000 /2 = 24 000/2 = 12K   
...
void MX_TIM17_Init(void)  
{
  ...
  htim17.Init.Prescaler = 384 -1; // 3840-1;
...
/* USER CODE BEGIN 1 */
#define TIM_us_Handle (&htim6)

//传入参数不能大于 所用定时器的Period 
void Delay_us(volatile  uint16_t nus)    // 阻塞延时 us级
{
	__HAL_TIM_SET_COUNTER(TIM_us_Handle, 0);   // 清零
	__HAL_TIM_ENABLE(TIM_us_Handle);					 // 开启计数
	while (__HAL_TIM_GET_COUNTER(TIM_us_Handle) < nus)   //等待计数到达
	{
	}
	__HAL_TIM_DISABLE(TIM_us_Handle);					//  关闭计数
}
